...
void icmpreq(struct sr_instance *sr, uint8_t *packet, unsigned int len, uint8_t type, uint8_t code, char *interface){
  ...
  newicmpheader[0].icmp_type=type;
  newicmpheader[0].icmp_code=code;
...
void sr_handlepacket(struct sr_instance* sr,
  ...
  if(ntohs(etheader->ether_type) ==0x800){
    ...
    { 
      ...
      ppinterface =  sr_get_interface(sr, temp1->interface);
    ...
    if(ipheader->ip_ttl==1){
      icmpreq(sr, packet, len, 11, 0, interface);
      return;
    }
    ...
    if(check==1){
      if(ipheader->ip_p==ip_protocol_icmp){
        ...
        icmpreq(sr, packet, len, 3, 3, interface);
        return;
      }
    }

    if(rt==NULL){
      icmpreq(sr, packet, len, 3, 0, interface);
      return; 
    }
  ...
  /* If it is an ARP request or Reply */
